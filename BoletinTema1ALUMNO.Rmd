...
# Los animales que estan en ambos conjuntos
...
```{r}
diferentes.m <- setdiff(rownames(df.m),rownames(df.A))
# Animales que no estan en ambos conjuntos
diferentes.m
...
diferentes.A <- setdiff(rownames(df.A),rownames(df.m))
diferentes.A
...
```{r}
# Hay dos formas de encontrar los elementos que no estan en ambos conjuntos, el orden de las variables en la funcion setdiff(x,y) cambia el resultado de los elementos.
...
# Ambos graficos cumplen la condicion de distribucion Gaussiana, por tanto, ambos tienen una distribucion Gaussiana
# Si que presentan el mismo comportamiento, aunque los puntos de Animals2 son menores que los de mammals
...
# Numero de librerias instaladas:
length(libReturn)
## 3

# Los 5 primeros elementos de libReturn:
# Cortamos la matriz
libReturn[[2]][1:5]
...
# b
any(is.na(cabbages))
# No hay valores perdidos en el conjunto cabbages
...
# Los datos siguen una distribución Gaussiana: los datos estan entre las lineas punteadas
...
```{r}
# a
# Diagrama de Dispersion Global:
plot(cabbages$HeadWt,cabbages$VitC)

# b
# Diagrama de dispersion etiquetando segun un factor:
plot(cabbages$VitC,cabbages$HeadWt, pch=c(6,16)[cabbages$Date])
...
```{r}
# Observamos los coeficientes del Model1
summary(Model1$coefficients)

# Observamos los coeficientes del Model2
summary(Model2$coefficients)


# Para calcular las pendientes:
lm(Model1)
# Vemos que la pendiente del modelo 1 es negativa

lm(Model2)
# Vemos que la pendiente del modelo 2 es negativa
